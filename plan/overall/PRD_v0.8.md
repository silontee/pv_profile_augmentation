# PRD — 태양광 발전소 프로파일 증강 파이프라인 v0.8
...
## 2. v0.8 변경 요약 (v0.7 대비)

| 항목 | 변경 내용 |
|---|---|
| 디렉토리 구조 | `source/` → `data/` 리네임 |
| UI Track 3 | v0.1 → **v0.2** (3D지형, 토지피복, 필터동기화) |
| 신규 데이터 소스 | 환경부 EGIS 토지피복지도 (WFS, 중분류 lv2_2025y) |
| 신규 크롤러 | `land_cover_crawler.py` (EGIS WFS 오프라인 수집) |
| FastAPI | `/api/landcover` 엔드포인트 추가 (EGIS 프록시) |
| 의존성 | `httpx` 추가 (비동기 HTTP 클라이언트) |
...
- **`land_cover_crawler.py`** (v0.8 신규): 환경부 EGIS WFS 토지피복 오프라인 수집
...
### Track 3. UI 시각화 — **v0.2 완료**

- 풀스택 Docker 구성 (PostgreSQL+PostGIS / FastAPI / React+MapLibre)
- 인터랙티브 지도: 시군구 클러스터 + 개별 마커 (zoom 10 전환)
- 검색·필터 패널, 발전소 상세 패널, 레이어 토글
- **[v0.2]** 3D 지형: MapTiler DEM + hillshade + pitch 50°
- **[v0.2]** 토지피복: EGIS WFS → FastAPI 프록시 → 7색 벡터 폴리곤
- **[v0.2]** 토지피복 범례 (우상단 오버레이)
- **[v0.2]** 필터 → 지도 동기화
...
src/
  crawlers/
    ...
    land_cover_crawler.py          # v0.8 신규
  preprocessing/preprocess.py
  ui/
    docker-compose.yml             # DB 포트: 5433
    backend/app/api/
      facilities.py / infra.py / stats.py
      landcover.py                 # v0.8 신규
    frontend/src/components/map/
      MapView.tsx                  # 3D지형·토지피복 추가
      LayerToggle.tsx              # 버튼 추가
      LandcoverLegend.tsx          # v0.8 신규
  data/                            # v0.8 리네임 (구 source/)
