...
데이터 소스:
    - parquet: src/data/processed/pv_facility_processed.parquet
    - 변전소:  src/data/openinframap/by_sido/substations_*.geojson
    - 송배전선: src/data/openinframap/by_sido/power_lines_*.geojson
"""
import glob
import json
import logging
...
if _DATA_ROOT:
    ...
    _PARQUET_PATH = _PROJECT_ROOT / "src" / "data" / "processed" / "pv_facility_processed.parquet"
    _SUBSTATIONS_GLOB = str(_PROJECT_ROOT / "src" / "data" / "openinframap" / "by_sido" / "substations_*.geojson")
    _POWER_LINES_GLOB = str(_PROJECT_ROOT / "src" / "data" / "openinframap" / "by_sido" / "power_lines_*.geojson")
