...
```bash
# 기본 실행 (headless 브라우저)
python3 src/crawlers/data_go_kr_solar_download_playwright.py --headless
```

- 상태 파일(`pv_facility_profile_state.csv`)로 수정일 비교 → 변경분만 다운로드
- 출력: `src/source/raw_csv/`
...
```bash
# 전국 수집
python3 src/crawlers/recloud_solar_crawler.py

# 특정 시도만
python3 src/crawlers/recloud_solar_crawler.py --sido 28

# 출력 디렉토리 변경
python3 src/crawlers/recloud_solar_crawler.py --output-dir src/data/recloud
```

- 출력: `src/data/recloud/` (sido_summary + gugun_detail CSV)
...
```bash
# 전국 수집
python3 src/crawlers/openinframap_power_crawler.py

# 특정 시도만
python3 src/crawlers/openinframap_power_crawler.py --sido 서울특별시

# 출력 디렉토리 변경
python3 src/crawlers/openinframap_power_crawler.py --output-dir src/data/openinframap
```
...
- 출력: `src/data/openinframap/` (substations + power_lines GeoJSON)
...
python3 src/crawlers/visualize_infra_map.py --data-dir src/data/openinframap

# 브라우저에서 바로 열기
open src/data/openinframap/infra_map.html

# 파일 직접 지정
python3 src/crawlers/visualize_infra_map.py \
  --substations src/data/openinframap/substations_XXXXXX.geojson \
  --lines src/data/openinframap/power_lines_XXXXXX.geojson

# 정적 이미지로 출력 (matplotlib, geopandas 필요)
python3 src/crawlers/visualize_infra_map.py \
  --data-dir src/data/openinframap \
  --output src/data/openinframap/infra_map.png
```

- `.html` 출력: folium 인터랙티브 맵 (어두운 배경, 전압별 색상/굵기 구분)
- `.png` 출력: matplotlib 정적 이미지
- 출력: `src/data/openinframap/infra_map.html`

### 지도 미리보기

> 아래 HTML 파일을 브라우저에서 열면 인터랙티브 지도를 확인할 수 있다.
>
> **[`src/data/openinframap/infra_map.html`](data/openinframap/infra_map.html)**
