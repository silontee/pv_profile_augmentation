...
def extract_modified_date(link) -> str:
    try:
        ...
        return ""
    match = re.search(r"?˜ì •??s*:\s*(\d{4}-\d{2}-\d{2})", text)
    return match.group(1) if match else ""
...
def main() -> None:
    ...
    parser.add_argument("--download-dir", default="src/data/raw_csv")
    parser.add_argument("--log-path", default="src/data/logs/crawler/download_log_playwright.csv")
    parser.add_argument("--state-path", default="src/data/logs/crawler/pv_facility_profile_state.csv")
